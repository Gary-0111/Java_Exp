...
class ServerThread extends Thread {
	...
	public void run(){
		try{
			...
			String str = in.readLine() + " entered the room.";
			...
			list.remove(socket);
			System.out.println(socket.getRemoteSocketAddress().toString().split("/")[1] + " left the room.");
			socket.close();
		}
		catch (Exception e){
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			while(true) {
				...
				//System.out.println(socket.getRemoteSocketAddress().toString().split("/")[1] + " entered the room.");
